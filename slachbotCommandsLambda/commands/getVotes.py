...
def GetDebugRoomVotes(event):
  ...
  lambdaResponse = client.invoke(
    FunctionName='arn:aws:lambda:us-east-2:145035066069:function:getValueFromSupergoonTable',
    Payload=data)
  items = json.loads(lambdaResponse.get('Payload').read()).get('Items')
  falseValues = [item for item in items if item.get('voteType') == Decimal(0)][0].get('votes')
  trueValues = [item for item in items if item.get('voteType') == Decimal(1)][0].get('votes')
